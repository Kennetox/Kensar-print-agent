...
const FIRE_AND_FORGET = process.env.FIRE_AND_FORGET === "1";
...
const FALLBACK_CONFIG_DIR = path.join(process.cwd(), ".agent-data");
let CONFIG_DIR = process.env.KENSAR_AGENT_CONFIG_DIR || DEFAULT_CONFIG_DIR;
...
function loadConfig() {
  ...
  try {
    ...
    return {
      selectedPrinterUrl: normalizePrinterUrl(
        parsed.selectedPrinterUrl || DEFAULT_PRINTER_URL
      ),
  ...
  try {
    ...
    if (upstream.status >= 200 && upstream.status < 300) {
      return res.json({
        ok: true,
        printerUrl,
        note: upstream.status === 202 ? "fire-and-forget" : undefined,
      });
    }
...
app.listen(PORT, () => {
  console.log(`Print agent running on http://localhost:${PORT}`);
  console.log(`Selected printer: ${agentConfig.selectedPrinterUrl}`);
  console.log(`Config file: ${getConfigPath()}`);
  if (FIRE_AND_FORGET) {
    console.log("Fire-and-forget enabled (will not wait for printer response).");
  }
});
